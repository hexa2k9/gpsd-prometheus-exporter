...
""" % (
    ...
    try:
        ...
        parser.add_argument(
            ...
            dest="geo_offset",
            default=False,
            help="track offset (x,y offset and distance) from a stationary location.",
        )
        parser.add_argument(
            "--geopoint-lat",
            dest="geo_lat",
            type=float,
            default=False,
            help="Latitude of a fixed stationary location.",
        )
        parser.add_argument(
            "--geopoint-lon",
            dest="geo_lon",
            type=float,
            default=False,
            help="Longitude of a fixed stationary location.",
        )
        ...
        if debug > 0:
            logging.basicConfig(
                format="DEBUG %(funcName)s(%(lineno)s): %(message)s",
                stream=sys.stderr,
                level=logging.DEBUG,
            )
        elif verbose:
...
def init_metrics(args):
    ...
    metrics["SKY"] = {
        ...
        "hdop": Gauge(
            "gpsd_hdop", "Horizontal dilution of precision", registry=registry
        ),
        "pdop": Gauge(
            "gpsd_pdop",
        ...
        "vdop": Gauge(
            "gpsd_vdop", "Vertical (altitude) dilution of precision", registry=registry
        ),
        "ydop": Gauge(
            "gpsd_ydop", "Longitudinal dilution of precision", registry=registry
        ),
        "xdop": Gauge(
            "gpsd_xdop", "Latitudinal dilution of precision", registry=registry
        ),
        "nSat": Gauge(
            "gpsd_nSat",
    ...
    metrics["TPV"] = {
        ...
        "altHAE": Gauge(
            "gpsd_altHAE",
            "Altitude, height above allipsoid, in meters. Probably WGS84.",
            registry=registry,
        ),
        "altMSL": Gauge(
            "gpsd_altMSL",
        ...
        "status": Gauge(
            "gpsd_status",
            "GPS fix status: %d, 2=DGPS fix, 3=RTK Fixed point, 4=RTK Floating point, 5=DR fix, 6=GNSSDR"
            + "fix, 7=Time (surveyed) fix, 8=Simulated, 9=P(Y) fix, otherwise not present. ",
            registry=registry,
        ),
        "leapseconds": Gauge(
            "gpsd_leapseconds", "Current leap seconds.", registry=registry
        ),
        "magvar": Gauge(
            "gpsd_magvar", "Magnetic variation, degrees.", registry=registry
        ),
        "ept": Gauge(
            "gpsd_ept", "Estimated timestamp error in seconds.", registry=registry
        ),
        "epx": Gauge(
            "gpsd_epx", "Longitude error estimate in meters.", registry=registry
        ),
        "epy": Gauge(
            "gpsd_epy", "Latitude error estimate in meters.", registry=registry
        ),
        "epv": Gauge(
            "gpsd_epv", "Estimated vertical error in meters.", registry=registry
        ),
        "eps": Gauge(
            "gpsd_eps", "Estimated speed error in meters per second.", registry=registry
        ),
        "epc": Gauge(
            "gpsd_epc", "Estimated climb error in meters per second.", registry=registry
        ),
        "geoidSep": Gauge(
            "gpsd_geoidSep",
        ...
        "eph": Gauge(
            "gpsd_eph",
            "Estimated horizontal Position (2D) Error in meters. Also known as Estimated Position Error (epe).",
            registry=registry,
        ),
        "sep": Gauge(
            "gpsd_sep",
        ...
        "ecefvx": Gauge(
            "gpsd_ecefvx", "ECEF X velocity in meters per second.", registry=registry
        ),
        "ecefvy": Gauge(
            "gpsd_ecefvy", "ECEF Y velocity in meters per second.", registry=registry
        ),
        "ecefvz": Gauge(
            "gpsd_ecefvz", "ECEF Z velocity in meters per second.", registry=registry
        ),
        "ecefpAcc": Gauge(
            "gpsd_ecefpAcc",
        ...
        "velN": Gauge(
            "gpsd_velN", "North velocity component in meters.", registry=registry
        ),
        "velE": Gauge(
            "gpsd_velE", "East velocity component in meters.", registry=registry
        ),
        "velD": Gauge(
            "gpsd_velD", "Down velocity component in meters.", registry=registry
        ),
    }

    metrics["USED"] = Gauge(
        "gpsd_sat_used", "Used in current solution? ", registry=registry
    )
    metrics["SEEN"] = Gauge(
        "gpsd_sat_seen", "Seen in current solution? ", registry=registry
    )
    metrics["VERSION"] = Info("gpsd_version", "Version Details", registry=registry)
    metrics["DEVICES"] = Info(
        "gpsd_devices", "Device Details", ["device"], registry=registry
    )
    metrics["SAT_STATUS"] = {}
    ...
    if args.pps:
        ...
        [
            PPS_BUCKETS.append(i * args.pps_bucket_size)
            for i in range(
                int(args.pps_bucket_count / -2), int(args.pps_bucket_count / 2) + 1
            )
        ]
        PPS_BUCKETS.append(float("inf"))
        metrics["PPS_HIS"] = Histogram(
            "gpsd_pps_histogram",
            "PPS Histogram",
            ["device"],
            buckets=PPS_BUCKETS,
    ...
    if args.geo_offset:
        GEO_BUCKETS_OFFSET = []
        [
            GEO_BUCKETS_OFFSET.append(i * args.geo_bucket_size)
            for i in range(1, args.geo_bucket_count)
        ]
        ...
        [
            ...
            for i in range(
                int(args.geo_bucket_count / -2), int(args.geo_bucket_count / 2) + 1
            )
        ]
        GEO_BUCKETS_YX.append(float("inf"))

        metrics["GEO_OFFSET"] = Histogram(
            "gpsd_geo_offset_m_histogram",
            "Geo offset Histogram (meters)",
            buckets=GEO_BUCKETS_OFFSET,
            registry=registry,
        )
        metrics["GEO_OFFSET_Y"] = Histogram(
            "gpsd_geo_bearing_x_histogram",
            "Y offset in meters from static geo point",
            buckets=GEO_BUCKETS_YX,
            registry=registry,
        )
        metrics["GEO_OFFSET_X"] = Histogram(
...
def getPositionData(gpsd, metrics, args):
    ...
    if args.debug > 1:
        log.debug(f'recieved {nx["class"]}: {nx}')

    if nx["class"] == "VERSION":

        metrics["VERSION"].info(
            {
                "release": nx["release"],
                "rev": nx["rev"],
            ...
            metrics["DEVICES"].labels(device["path"]).info(
                {
                    ...
                    "activated": (
                        device["activated"] if "activated" in device else "Unknown"
                    ),
                    "flags": str(device["flags"]) if "flags" in device else "Unknown",
                    "native": (
                        str(device["native"]) if "native" in device else "Unknown"
                    ),
                    "bps": str(device["bps"]) if "bps" in device else "Unknown",
                    "parity": (
                        str(device["parity"]) if "parity" in device else "Unknown"
                    ),
                    "stopbits": (
                        str(device["stopbits"]) if "stopbits" in device else "Unknown"
                    ),
                    "cycle": str(device["cycle"]) if "cycle" in device else "Unknown",
                    "mincycle": (
                        str(device["mincycle"]) if "mincycle" in device else "Unknown"
                    ),
                }
            )
        ...
        if args.geo_offset:
            if hasattr(nx, "lat") and hasattr(nx, "lon"):
                offset = MeterOffsetSmall(
                    (args.geo_lat, args.geo_lon), (nx["lat"], nx["lon"])
                )
                distance = gps.misc.EarthDistanceSmall(
                    (nx["lat"], nx["lon"]), (args.geo_lat, args.geo_lon)
                )

                log.debug(
                    f"distance {distance:0.2f}m offset x = {offset[0]:0.2f}m y = {offset[1]:0.2f}m"
                )
    ...
    def collect(self):
        ...
        metrics = {
            "ss": GaugeMetricFamily(
                ...
                labels=["PRN", "svid", "gnssid", "used"],
            ),
            "az": GaugeMetricFamily(
                "gpsd_sat_az",
                "Azimuth, degrees from true north.",
                labels=["PRN", "svid", "gnssid", "used"],
            ),
            "el": GaugeMetricFamily(
                "gpsd_sat_el",
                "Elevation in degrees.",
                labels=["PRN", "svid", "gnssid", "used"],
            ),
            "used": GaugeMetricFamily(
                "gpsd_used", "Used Satilite", labels=["PRN", "svid", "gnssid", "used"]
            ),
...
def add_sat_stats(satellites):
    ...
    log.debug(
        f"Current satellite queue size = {q_size_end}, pruned {q_size - q_size_end}"
    )
